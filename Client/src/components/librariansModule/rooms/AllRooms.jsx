...
function AllRooms() {
  ...
  const { data, isLoading } = useGetLibrarianRoomsQuery();
    ...
    <div className="space-y-6 w-full">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-accent">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-2xl font-bold text-dark">Meeting Rooms</h2>
        </div>
        <Button
        ...
        <Table className="table-fixed w-full">
          <TableHeader className="bg-accent/30">
            <TableRow>
              ...
              <TableHead className="text-right font-semibold text-primary w-[25%]">
                Actions
              </TableHead>
            </TableRow>
          </TableHeader>
              ...
              paginatedData.map((room) => (
                ...
                >
                  ...
                  <TableCell className="w-[20%]">
                    <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-secondary/20 text-secondary">
                      {room.availableTimeSlots?.length || 0} slots
                    </span>
                  </TableCell>
                  <TableCell className="text-dark w-[20%]">
                  ...
                  <TableCell className="text-right w-[25%]">
                    <div className="flex items-center justify-end space-x-2">
                      ...
                      <Button
                        variant="ghost"
                        size="icon"
                        onClick={() => navigate(`form`, { state: room })}
                        className="hover:bg-primary/20 hover:text-primary"
                        title="Edit room"
                      >
              ...
              <TableRow>
                <TableCell colSpan={5} className="text-center py-8">
                  <div className="flex flex-col items-center justify-center">
                    <DoorOpen size={32} className="text-primary/70" />
                    <h3 className="text-lg font-semibold mt-2 text-dark">
                      No rooms found
                    </h3>
                    <p className="mt-1 text-sm text-dark/80">
                      Try adjusting your search parameters to find rooms.
                    </p>
                  </div>
                </TableCell>
              </TableRow>
